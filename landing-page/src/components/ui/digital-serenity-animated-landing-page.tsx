...
const DigitalSerenity = () => {
  const [mouseGradientStyle, setMouseGradientStyle] = useState<GradientStyle>({ left: '0px', top: '0px', opacity: 0 });
  const [ripples, setRipples] = useState<Ripple[]>([]);
  const [hasStarted, setHasStarted] = useState(false); // 仅当滚动到视口或接收到点击事件后启动
  const floatingElementsRef = useRef<HTMLElement[]>([]);
  const containerRef = useRef<HTMLDivElement | null>(null);
  const { ref: visibilityRef, isVisible } = useScrollTrigger<HTMLDivElement>({ threshold: 0.2 });
  // 合并可见性 ref 与本地 DOM ref
  const setRefs = (el: HTMLDivElement | null) => {
    containerRef.current = el;
    (visibilityRef as MutableRefObject<HTMLDivElement | null>).current = el;
  };
  
  // 监听自定义事件与可见性，启动动画
  useEffect(() => {
    if (isVisible && !hasStarted) setHasStarted(true);
  }, [isVisible, hasStarted]);

  useEffect(() => {
    const handleStart = () => setHasStarted(true);
    window.addEventListener('digital-serenity:start', handleStart as unknown as EventListener);
    return () => {
      window.removeEventListener('digital-serenity:start', handleStart as unknown as EventListener);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const elements = scope.querySelectorAll('.floating-element-animate');
  ...
  // 文本浮现动画（提示词中的 CSS word-appear），仅启动一次
  useEffect(() => {
    if (!hasStarted || !containerRef.current) return;
    const wordElements = containerRef.current.querySelectorAll('.word-animate');
    wordElements.forEach(word => {
      const delay = parseInt((word as HTMLElement).getAttribute('data-delay') || '0', 10) || 0;
      setTimeout(() => {
  ...
  return (
    <>
      ...
      <div ref={setRefs} className="min-h-screen bg-gradient-to-br from-slate-900 via-black to-slate-800 text-slate-100 font-primary overflow-hidden relative">
        {hasStarted && (
          <>
            <svg className="absolute inset-0 w-full h-full pointer-events-none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
              ...
              <line x1="0" y1="20%" x2="100%" y2="20%" className="grid-line" style={{ animationDelay: '0.5s' as unknown as string }} />
              <line x1="0" y1="80%" x2="100%" y2="80%" className="grid-line" style={{ animationDelay: '1s' as unknown as string }} />
              <line x1="20%" y1="0" x2="20%" y2="100%" className="grid-line" style={{ animationDelay: '1.5s' as unknown as string }} />
              <line x1="80%" y1="0" x2="80%" y2="100%" className="grid-line" style={{ animationDelay: '2s' as unknown as string }} />
              <line x1="50%" y1="0" x2="50%" y2="100%" className="grid-line" style={{ animationDelay: '2.5s', opacity: '0.05' } as unknown as React.CSSProperties} />
              <line x1="0" y1="50%" x2="100%" y2="50%" className="grid-line" style={{ animationDelay: '3s', opacity: '0.05' } as unknown as React.CSSProperties} />
              <circle cx="20%" cy="20%" r="2" className="detail-dot" style={{ animationDelay: '3s' as unknown as string }} />
              <circle cx="80%" cy="20%" r="2" className="detail-dot" style={{ animationDelay: '3.2s' as unknown as string }} />
              <circle cx="20%" cy="80%" r="2" className="detail-dot" style={{ animationDelay: '3.4s' as unknown as string }} />
              <circle cx="80%" cy="80%" r="2" className="detail-dot" style={{ animationDelay: '3.6s' as unknown as string }} />
              <circle cx="50%" cy="50%" r="1.5" className="detail-dot" style={{ animationDelay: '4s' as unknown as string }} />
            </svg>
            ...
            <div className="relative z-10 min-h-screen flex flex-col justify-between items-center px-6 py-10 sm:px-8 sm:py-12 md:px-16 md:py-20">
              <div className="text-center">
                <h2 className="text-xs sm:text-sm font-mono font-light text-slate-300 uppercase tracking-[0.2em] opacity-80">
                  ...
                  <span className="word-animate" data-delay="400">PHILOSOPHY</span>
                </h2>
              ...
              <div className="text-center max-w-5xl mx-auto relative">
                <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold leading-tight tracking-tight text-slate-50 text-decoration-animate">
                  <div className="mb-6 md:mb-8 whitespace-nowrap">
                    {"记忆的第一性原理".split('').map((ch, idx) => (
                      <span key={idx} className="word-animate" data-delay={`${700 + idx * 80}`}>{ch}</span>
                    ))}
                  </div>
                  <div className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-semibold text-slate-200 leading-relaxed tracking-wide text-right pr-8 md:pr-12 lg:pr-20">
                    <span className="word-animate" data-delay="2000">——创建链接</span>
                  </div>
                </h1>
              ...
              <div className="text-center">
                ...
                <h2 className="text-[10px] sm:text-xs font-mono font-light text-slate-400 uppercase tracking-[0.3em] opacity-80">
                  <span className="word-animate" data-delay="3200">FLOW</span>
                  <span className="word-animate" data-delay="3400">STATE</span>
                  <span className="word-animate" data-delay="3600">LEARNING</span>
                </h2>
